...
<template>
  <section v-if="user" class="profile card">
    <img class="profile__avatar profil__avatar--desktop" :src="user.avatar_url" alt="" />
    <div>
      <div class="profile__header">
        <img class="profile__avatar profil__avatar--mobile" :src="user.avatar_url" alt="" />
        <div>
          <h2 class="profile__name">{{ user.name }}</h2>
          <h3 class="profile__pseudo">@{{ user.login }}</h3>
          <span class="profile__creation-date"
            >Joined
            {{
      ...
      <div class="profile__inner">
        ...
        <div class="profile__stats">
          ...
          <div>
            <h4 class="stat__title">Followers</h4>
            <span class="stat__value">{{ user.followers }}</span>
          </div>
          <div>
            <h4 class="stat__title">Following</h4>
            <span class="stat__value">{{ user.following }}</span>
          </div>
        </div>
      </div>
      <div class="profile__details">
        <div :style="{ opacity: user.location ? '1' : '0.5' }">
        ...
        <div :style="{ opacity: user.blog ? '1' : '0.5' }">
          ...
          <a v-if="user.blog" href="#" target="_blank">{{ user.blog }}</a>
          <span v-else>Not Available</span>
        </div>
        <div :style="{ opacity: user.twitter_username ? '1' : '0.5' }">
        ...
        <div :style="{ opacity: user.company ? '1' : '0.5' }">
          <svg height="20" width="20" xmlns="http://www.w3.org/2000/svg">
            <g fill="currentColor">
              <path
...
.profile {
  ...
  .profil__avatar--mobile {
    @include mixins.laptopAndUp {
      display: none;
    }
  }
  .profil__avatar--desktop {
    display: none;
